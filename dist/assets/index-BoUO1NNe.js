const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/web-uq0woMPe.js","assets/index-B40g6gKq.js","assets/index-Dt5N_hgV.js","assets/index-C4y5DRbr.css"])))=>i.map(i=>d[i]);
import { _ as __vitePreload } from "./index-Dt5N_hgV.js";
import { registerPlugin } from "./index-B40g6gKq.js";
var ImpactStyle;
(function(ImpactStyle2) {
  ImpactStyle2["Heavy"] = "HEAVY";
  ImpactStyle2["Medium"] = "MEDIUM";
  ImpactStyle2["Light"] = "LIGHT";
})(ImpactStyle || (ImpactStyle = {}));
var NotificationType;
(function(NotificationType2) {
  NotificationType2["Success"] = "SUCCESS";
  NotificationType2["Warning"] = "WARNING";
  NotificationType2["Error"] = "ERROR";
})(NotificationType || (NotificationType = {}));
const Haptics = registerPlugin("Haptics", {
  web: () => __vitePreload(() => import("./web-uq0woMPe.js"), true ? __vite__mapDeps([0,1,2,3]) : void 0).then((m) => new m.HapticsWeb())
});
export {
  Haptics,
  ImpactStyle,
  NotificationType
};
