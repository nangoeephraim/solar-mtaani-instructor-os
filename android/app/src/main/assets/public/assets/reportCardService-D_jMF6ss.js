import { E } from "./jspdf.es.min-op3uTeDl.js";
import "./index-l2RTGEA9.js";
...
function generateReportCard(student, data, options) {
...
async function generateBulkReportCards(students, data, options) {
  const opts = { ...DEFAULT_OPTIONS, ...options };
  const doc = new E("p", "mm", "a4");
  const pageWidth = doc.internal.pageSize.getWidth();
  const margin = 15;
  ...
  const fileName = `Report_Cards_Term${opts.term}_${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.pdf`;
  doc.save(fileName);
}
function drawSchoolHeader(doc, opts, pageWidth, margin, y) {
  doc.setFillColor(...COLORS.primary);
...
function drawStudentInfo(doc, student, opts, pageWidth, margin, y) {
  ...
  drawInfoRow(doc, "Year:", student.lot || (/* @__PURE__ */ new Date()).getFullYear().toString(), col1, y + 6 + rowH * 3);
...
function hasKNECAssessments(student) {
  var _a2;
  if (!((_a2 = student.assessment) == null ? void 0 : _a2.units)) return false;
  return Object.values(student.assessment.units).some((u) => u.system === "KNEC");
}
function drawKNECTable(doc, student, _data, _pageWidth, margin, y) {
  var _a2, _b, _c, _d, _e;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  ...
  y += 8;
  const units = ((_a2 = student.assessment) == null ? void 0 : _a2.units) || {};
  const rows = [];
  let totalScore = 0;
  let count = 0;
  for (const [unitId, unit] of Object.entries(units)) {
    if (unit.system !== "KNEC") continue;
    const cat1 = ((_b = unit.cat1) == null ? void 0 : _b.score) ?? "-";
    const cat2 = ((_c = unit.cat2) == null ? void 0 : _c.score) ?? "-";
    const practical = ((_d = unit.practical) == null ? void 0 : _d.score) ?? "-";
    const finalExam = ((_e = unit.finalExam) == null ? void 0 : _e.score) ?? "-";
  ...
  return doc.lastAutoTable.finalY || y + 40;
}
function drawCBETTable(doc, student, _data, _pageWidth, margin, y) {
  var _a2;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  doc.setTextColor(...COLORS.primary);
...
function drawCompetencySummary(doc, student, pageWidth, margin, y) {
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.setTextColor(...COLORS.primary);
  ...
  const barHeight = 5;
  const labels = { 1: "Emerging", 2: "Developing", 3: "Competent", 4: "Mastered" };
  const barColors = [
    COLORS.danger,
    COLORS.warning,
    [59, 130, 246],
...
function drawAttendanceSection(doc, student, pageWidth, margin, y) {
  var _a2, _b;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.setTextColor(...COLORS.primary);
  ...
  const totalDays = ((_a2 = student.attendanceHistory) == null ? void 0 : _a2.length) || 0;
  const presentDays = ((_b = student.attendanceHistory) == null ? void 0 : _b.filter((a) => a.status === "present").length) || 0;
...
function drawRemarksSection(doc, student, opts, pageWidth, margin, y) {
  var _a2;
  if (y > 250) {
    doc.addPage();
    y = 20;
  ...
  const remarks = ((_a2 = student.notes) == null ? void 0 : _a2.length) ? student.notes[student.notes.length - 1] : "Good progress. Keep up the effort.";
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.setTextColor(...COLORS.dark);
...
function drawFooter(doc, opts, pageWidth) {
  ...
  doc.setTextColor(...COLORS.white);
  doc.text(
    `${opts.schoolName} • This is a computer-generated report card • Generated on ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-GB")}`,
    pageWidth / 2,
    pageHeight - 3,
    { align: "center" }
  );
