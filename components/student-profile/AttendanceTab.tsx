...
export const AttendanceTab: React.FC<AttendanceTabProps> = ({ student, onAddNote }) => {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fade-in relative z-10">
            ...
            <div className="lg:col-span-8 glass-panel p-6 rounded-2xl flex flex-col h-[600px] relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-violet-400 to-fuchsia-500 opacity-80" />

                <h3 className="text-xs font-black text-violet-600 dark:text-violet-400 uppercase tracking-widest mb-6 flex items-center gap-2 font-google">
                    <Activity size={16} /> Behavior & Interaction Timeline
                </h3>

                {/* Input Area - Wrap in input-glow */}
                <div className="flex gap-3 mb-8 input-glow rounded-xl border border-[var(--md-sys-color-outline)] transition-all bg-[var(--md-sys-color-surface-variant)]/40 p-2 backdrop-blur-sm shadow-sm relative z-10">
                    <input
                        type="text"
                        ...
                        <div className="space-y-6 pb-4">
                            {student.notes.map((note, i) => (
                                <div key={i} className="relative flex gap-4 group">
                                    {/* Timeline Marker */}
                                    <div className="relative z-10 w-8 h-8 rounded-full bg-violet-100 dark:bg-violet-900/30 border-2 border-[var(--md-sys-color-surface)] flex items-center justify-center text-violet-600 dark:text-violet-400 shadow-sm mt-1 ring-4 ring-[var(--md-sys-color-surface)]/20 flex-shrink-0 group-hover:scale-110 transition-transform">
                                        <MessageSquare size={14} />
                                    </div>

                                    {/* Content Card - Glass styled */}
                                    <div className="flex-1 glass-card p-4 rounded-2xl shadow-sm border border-[var(--md-sys-color-outline)]/65 group-hover:border-violet-300 dark:group-hover:border-violet-800/80 group-hover:shadow-md transition-all duration-300 relative overflow-hidden">
                                        <div className="absolute left-0 top-0 bottom-0 w-1 bg-violet-500/25 group-hover:bg-violet-550 transition-all duration-300" />

                                        <div className="flex justify-between items-center mb-2">
                                            <div className="flex items-center gap-2 text-[10px] font-bold text-violet-600 dark:text-violet-400 uppercase tracking-widest bg-violet-50/70 dark:bg-violet-500/10 px-2 py-0.5 rounded-full">
                                                Instructor Note
                                            </div>
            ...
            <div className="lg:col-span-4 space-y-6">
                <div className="glass-panel p-6 rounded-2xl h-[600px] relative overflow-hidden group hover:shadow-md transition-all duration-300">
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-400 to-sky-500 opacity-80" />

                    <h3 className="text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest mb-6 flex items-center gap-2 font-google">
                        <CalendarCheck size={16} /> Record Summary
                    </h3>

                    <div className="flex flex-col gap-4">
                        {/* Overall Attendance Metric */}
                        <div className="bg-emerald-50/60 dark:bg-emerald-900/10 border border-emerald-100/50 dark:border-emerald-500/20 rounded-2xl p-6 flex flex-col items-center justify-center relative overflow-hidden group hover:scale-[1.02] transition-all duration-300">
                        ...
                        {/* Present Count */}
                        <div className="p-4 glass-card rounded-2xl flex items-center justify-between border-[var(--md-sys-color-outline)]/60 shadow-sm hover:shadow-md transition-all duration-300 hover:scale-[1.02]">
                            <div>
                        ...
                        <div className="p-4 glass-card rounded-2xl flex items-center justify-between border-[var(--md-sys-color-outline)]/60 shadow-sm hover:shadow-md transition-all duration-300 hover:scale-[1.02]">
                            <div>
                                <p className="text-[10px] uppercase tracking-widest font-bold text-[var(--md-sys-color-secondary)] flex items-center gap-1.5">
                        ...
                        {/* Notice */}
                        {absentCount > 3 && (
                            <div className="mt-4 p-4 bg-rose-50 border border-rose-100 dark:bg-rose-900/10 dark:border-rose-800/20 rounded-xl flex gap-3 text-rose-700 dark:text-rose-400 animate-pulse">
